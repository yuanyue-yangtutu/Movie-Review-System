    ...
    return (node, context) => {
        ...
        if (templateAstTypes.isComponentNode(node)) {
            if (!['component'].includes(node.tag)) {
                const name = shared.pascalCase(node.tag);
...
function generateVOn(dir, node) {
    ...
    if (templateAstTypes.isSimpleExpressionNode(dir.arg)) {
        if (dir.arg.isStatic) {
            code.push(compilerCore.createSimpleExpression(shared.camelCase('on-' + dir.arg.content), false, dir.arg.loc), '={', ...exp, '}');
        }
        else {
            code.push('{...({[', dir.arg, ']: ', ...exp, '})}');
        }
...
function compile(template, options) {
    ...
    const config = {
        ...
        components: {
            ...components,
            ...options.components,
            [shared.getComponentName(options.filename)]: {
