...
function asFsPath(uri) {
    if (isUri(uri)) {
        return replaceSlashes(vscodeUri.uriToFsPath(vscodeUri.URI.parse(uri), true));
    }
...
function createVueModuleTextDocument(options) {
    return TransformedBlockTextDocument.create({
        ...
        transformer: (document) => {
            ...
            const name = shared.getComponentName(document.container.fsPath);
            if (script != null) {
...
class VueTextDocument extends ProxyTextDocument {
    ...
    all() {
        this.parse();
        return Array.from(this.documents.values()).filter(shared.isNotNull);
    }
    getBlock(selector) {
        this.parse();
    ...
    blockAt(position) {
        ...
        const offset = shared.isNumber(position) ? position : this.offsetAt(position);
    ...
    getDocumentFileName(selectorLike) {
        const selector = shared.isString(selectorLike)
            ? { type: selectorLike }
            : selectorLike;
